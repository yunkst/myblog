...
**最后更新**: 2026-08-28
**状态**: 修订稿 v3（新增 §5.6 视觉设计基准：工程图纸风格 + 架构图方案 A），待评审
...
### 5.6 视觉设计基准（v3 新增，已确认）

**概念：工程图纸。** 内容是部件，FAQ 是标注引线，动画是装配过程。视觉基准原型见 `docs/design/style-tile.html`（含三方案对比稿 `arch-diagram-styles.html`，已选定方案 A）。

**色板**（全站仅此 5 色 + 网格线）：

| Token | 值 | 用途 |
|---|---|---|
| `--paper` | `#F6F7F4` | 页面底色（冷纸白，带绿灰） |
| `--ink` | `#1C2B28` | 主文字 / 节点描边（墨绿黑） |
| `--ink-soft` | `#55665F` | 次级文字 / 常规连线 |
| `--ink-faint` | `#93A39C` | 弱标注 / mono 副标签 |
| `--accent` | `#0E6E5C` | 图纸青：链接、hover、进度、关键节点描边、装配动画 |
| `--mark` | `#C0392B` | 校对红：**仅** WIP 卡片左边线等关键标注，全页 ≤ 2 处 |

**字体角色**：

- **黑体**（PingFang SC / Noto Sans SC）：标题 + 正文
- **仿宋**（FangSong / STFangsong）：标注层 —— eyebrow、FAQ 问题、图注、批注、状态徽标（国标工程图字体，即本方案的辨识度来源）
- **等宽**（Cascadia Mono / Consolas）：编号、日期、技术参数副标签、连线标签

**版式**：

- 底层坐标纸网格：细格 24px，每 120px 加深一格（`repeating-linear-gradient`，opacity ~4.5%）
- 圆角全局 2px；卡片 = 纸白 + 1px 墨线 + hover 图纸青描边 + 上移 2px
- 首页 hero 下放**尺寸线**签名元素（生长动画 + mono 标注"年限 · 领域数 · 文章数"）

**架构图（方案 A 画法，吸收 archify）**：

- 节点 = 左上 sigil 线性图标 + 黑体主标签 + mono 副标签（技术参数）；关键节点图纸青描边加粗
- 连线 = 直角折线 + 箭头 marker + **遮罩药丸标签**（背景块盖线再写字）；强调线图纸青
- **边界区域**虚线框包裹相关节点（`应用层 · app` 等，仿宋标签）
- 图下方**要点卡**三段式（读路径/写路径/一致性，彩色圆点 + bullet）
- 装配动画：边界淡入 → 节点按依赖序出现 → 连线生长 → 标签浮现（ScrollTrigger 驱动；`prefers-reduced-motion` 时全部静态显示）
- 图数据用声明式结构（nodes/edges/bounds 数组）驱动渲染，文章内通过 MDX 组件传 props 使用
