...
export const figArchitecture: {
  nodes: ArchNode[]; edges: ArchEdge[]; bounds: ArchBound[]
  /** 默认左上角图例撞「接入层」贴纸，挪到底部走廊（y>420 全图无元素） */
  ...
  nodes: [
    ...
    { id: 'repo',     x: 440, y: 200, w: 140, h: 60, kind: 'db',       label: '协议仓库',            sub: '三档 + 可逆标记',               sigil: SIGIL_DB },
    { id: 'policy',   x: 620, y: 200, w: 140, h: 60, kind: 'key',      label: '分级策略',            sub: '直调 / 确认 / 预演',           sigil: SIGIL_ARROWS },
  ...
  edges: [
    ...
    /* e5/e6：平台直连业务后台（不绕回 Apisix），身份在请求头里透传。
     * e5 走 y300 行间通道到 be-b；e6 走 y270 通道（低于 repo 底边 y260）到 be-a */
    { id: 'e5', from: 'policy',   to: 'be-b',     fromSide: 'bottom', toSide: 'top', label: '5. 携带身份直连', emph: true, via: [{ x: 690, y: 300 }, { x: 505, y: 300 }] },
    { id: 'e6', from: 'policy',   to: 'be-a',     fromSide: 'bottom', toSide: 'top', label: '5. 携带身份直连', emph: true, via: [{ x: 660, y: 270 }, { x: 305, y: 270 }] },
    { id: 'e7', from: 'be-a',     to: 'rbac',     fromSide: 'right', toSide: 'bottom', label: '6. RBAC 裁决', via: [{ x: 405, y: 370 }, { x: 405, y: 425 }, { x: 690, y: 425 }] },
    { id: 'e8', from: 'be-b',     to: 'rbac',     fromSide: 'right', toSide: 'left', label: '6. RBAC 裁决' },
  ],
  bounds: [
...
export const figTiered: { nodes: ArchNode[]; edges: ArchEdge[]; bounds: ArchBound[] } = {
  nodes: [
    ...
    { id: 'e1',    x: 220, y: 240, w: 120, h: 60, kind: 'be',       label: '只读',               sub: '直调生产',                                   sigil: SIGIL_DOC },
    { id: 'e2',    x: 370, y: 240, w: 130, h: 60, kind: 'be',       label: '安全写',             sub: '确认 → 直打生产',                            sigil: SIGIL_WRITE },
    { id: 'e3',    x: 530, y: 240, w: 160, h: 60, kind: 'key',      label: '写 · 预演级',        sub: '测试服预演 → 生效 → 生产',                  sigil: SIGIL_ARROWS },
    { id: 'q2',    x: 40,  y: 360, w: 150, h: 60, kind: 'key',      label: '事后发现问题？',     sub: '决策',                                       sigil: SIGIL_ARROWS },
    { id: 'r1',    x: 220, y: 460, w: 120, h: 60, kind: 'be',       label: '一键撤回',           sub: '可逆接口 · 恢复原样',                        sigil: SIGIL_DOC },
    { id: 'r2',    x: 370, y: 460, w: 130, h: 60, kind: 'key',      label: '复盘会',             sub: '不可撤回 · 回放决策链',                      sigil: SIGIL_ARROWS },
    { id: 'r3',    x: 530, y: 460, w: 110, h: 60, kind: 'be',       label: '收工',               sub: '✓',                                          sigil: SIGIL_WRITE },
  ],
  edges: [
    ...
    { id: 'e6', from: 'e3',    to: 'q2',    fromSide: 'bottom', toSide: 'top', label: '生产后', via: [{ x: 610, y: 320 }, { x: 115, y: 320 }] },
...
export const figDevFlow: {
  nodes: ArchNode[]; edges: ArchEdge[]; bounds: ArchBound[]
  /** 单行流水线：默认左上角撞 bound，挪到节点排下方空带 */
  legendAt: { x: number; y: number }
} = {
  legendAt: { x: 12, y: 130 },
  nodes: [
  ...
  edges: [
    ...
    /* e9：跳过 m2 直连——原 y70 横线穿 m2 节点，改走 y130 下方通道绕开 */
    { id: 'e9',  from: 'te', to: 'u',   fromSide: 'right', toSide: 'left', dash: true, via: [{ x: 1082, y: 130 }, { x: 1218, y: 130 }] },
